...
if (empty($_POST['id_familia'])) {
    ...
    $query      = mysqli_query($conexion, "select * from miembros where familia_id='" . $id_familia . "'");
    $count      = mysqli_num_rows($query);
    if ($count == 0) {
        if ($delete1 = mysqli_query($conexion, "DELETE FROM familias WHERE id_familia='" . $id_familia . "'")) {
            ?>
