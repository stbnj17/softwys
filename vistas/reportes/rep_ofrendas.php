...
$sWhere .= " and ingresos.cod_ingreso=1";
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(300, 30, "Reporte de Ofrendas", 0, 1, 'C');
...
while ($row = mysqli_fetch_array($resultado)) {
    ...
    $referencia  = $row['ref_ingreso'];
    $total += $row['monto'];
    $tipo        = get_row('tipo_ingreso', 'nombre_tipoi', 'id_tipoi', $row['tipo_ingreso']);
    $miembro     = get_row('miembros', 'nombre_miembro', 'id_miembro', $row['miembro_id']) . ' ' . get_row('miembros', 'apellido_miembro', 'id_miembro', $row['miembro_id']);
    $pdf->Cell($w[0], 6, $row['ref_ingreso'], 1, 0, 'L', $bandera);
    ...
    $pdf->Cell($w[4], 6, pago2($row['pago_ingreso']), 1, 0, 'L', $bandera);
    $pdf->Cell($w[5], 6, ($moneda . ' ' . number_format($row['monto'], 4)), 1, 0, 'L', $bandera);
    $pdf->Ln();
