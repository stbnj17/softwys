...
function Header()
{
    ...
    // Logo
        ...
        $this->Cell(300, 30, "Reporte de Ventas", 0, 1, 'C');
...
$pdf->Cell(50,12,'USUARIO',1,0,'C',1);
...
while($row = mysqli_fetch_array($resultado))
    ...
    $users = $rw['nombre_users'].' '.$rw['apellido_users'];
        ...
        $pdf->Cell($w[1],6,$row['nombre_cliente'],1, 0 , 'L', $bandera);
        $pdf->Cell($w[2],6,($moneda.' '.formato($row['monto_factura'])),1, 0 , 'L', $bandera);
        $pdf->Cell($w[3],6,$estado,1,0 , 'L', $bandera);
        $pdf->Cell($w[4],6,$users,1, 0 , 'L', $bandera);
