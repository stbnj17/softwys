...
$tables    = "asistencias,  users";
$campos    = "*";
$sWhere    = "users.id_users=asistencias.users";
if ($cel > 0) {
...
$sWhere .= " GROUP BY month(fecha_add) order by month(fecha_add) asc";
// Consulta la manda a la siguiente pagina
$query = mysqli_query($conexion, "SELECT $campos FROM  $tables where $sWhere ");

// get the HTML
ob_start();
include dirname(__FILE__) . '/res/rep_ast_html.php';
...
try
{
    ...
    ob_end_clean();
    $html2pdf->Output('Asistencias.pdf');
