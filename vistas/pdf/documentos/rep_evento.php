...
$tables      = "events,  users";
$campos      = "*";
$sWhere      = "users.id_users=events.id_users";
if ($employee_id > 0) {
    $sWhere .= " and events.id_users = '" . $employee_id . "' ";
}
if (!empty($daterange)) {
    ...
    $sWhere .= " and events.start between '$fecha_inicial' and '$fecha_final' ";
}
$sWhere .= " order by events.id desc";
// Consulta la manda a la siguiente pagina
$query = mysqli_query($conexion, "SELECT $campos FROM  $tables where $sWhere ");
// get the HTML
ob_start();
include dirname(__FILE__) . '/res/rep_evento_html.php';
